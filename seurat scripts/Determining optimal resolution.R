...
# Create a shared nearest neighbor (SNN) graph
processed_LRP <- FindNeighbors(processed_LRP, assay = "RNA", force.recalc = TRUE)
...
# Define your list of resolutions, add additional result ions to create plots for those res of marker gene list.
resolutions <- c(2)


# Outer loop iterating over resolutions
for(res in resolutions) {
  # Create clusters for the current resolution using the integrated_snn graph
  processed_LRP <- FindClusters(processed_LRP, resolution = res, graph.name = "RNA_snn")
