...
get_gene_description_tair <- function(gene_id) {
  # Define the base URL
  base_url <- "https://www.arabidopsis.org/tools/bulk/genes/index.jsp"
  
  # Get the HTML content of the page
  session <- html_session(base_url)
  
  # Find the form on the page (in this case, the fifth form)
  form <- html_form(session)[[5]]  # Adjusted to the fifth form
  
  # Fill in the form with the gene ID
  filled_form <- set_values(form, 'search_for' = gene_id)
  
  # Submit the form
  session <- submit_form(session, filled_form)
  
  # Parse the HTML content of the page
  gene_description <- session %>%
    html_nodes("tr:nth-child(2) td:nth-child(3)") %>%  # CSS selector for the third <td> in the second <tr>
    html_text()
  
  # Since there might be multiple matches, we only return the first one
  if (length(gene_description) >= 1) {
    return(gene_description[1])
  } else {
    return(NA)
  }
}
...
for (i in 1:nrow(gene_info_data)) {
  ...
  gene_description <- get_gene_description_tair(gene_id)  # Fetch the gene description from tair
  ...
  png_file_name <- file.path(dir_name, paste0(cell_type, "_genes_", gene_id, "_", gene_name, ".png"))
