...
# Reshape this data frame from wide format to long format, ensuring one row per cluster
avg_gene_expression_df <- avg_gene_expression_df %>% 
  gather(key = "earlymappingbycelltype", value = gene_of_interest)

#reshape column to match column name in cc_df
avg_gene_expression_df$earlymappingbycelltype <- gsub("RNA.", "", avg_gene_expression_df$earlymappingbycelltype)

#Merge this with cc_df based on "earlymappingbycelltype".
...
# Print the plot
print(p)
filename <- paste0(gene_of_interest, ".png")
